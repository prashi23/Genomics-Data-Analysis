...
pdf("results/performance-plots-mgw.pdf");

data<-read.table("results/performance-ws-mgw.txt",header=TRUE,row.names=1)
fulldata<-read.table("results/cross-validation-full-ws5-mgw.txt",header=TRUE)

params<-c("shear", "stretch", "stagger", "buckle", "prop_tw", "opening", "shift", "slide", "rise", "tilt", "roll", "twist");
pnames<-c();
for (param  in "mgw")
{
